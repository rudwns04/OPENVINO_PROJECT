...
current_backgrounds = {emotion: [] for emotion in EMOTIONS}
for emotion, folder in backgrounds_folder.items():
    bg_images = os.listdir(folder)
    bg_image_paths = [os.path.join(folder, img) for img in bg_images]
    current_backgrounds[emotion] = bg_image_paths
...
while True:
    ...
    if len(faces) > 0:
        ...
        bg_image_path = random.choice(current_backgrounds[label])
